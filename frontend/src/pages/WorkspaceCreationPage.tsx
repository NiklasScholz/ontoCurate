...
import InviteMembersSection, { type PendingInvite } from "../components/InviteMembersPanel";
...
export default function WorkspaceCreationPage() {
    ...
    useEffect(() => {
        client.GET("/schemas/").then(({ data, error }) => {
            ...
            if (error || !schemaNames) { setSchemasError("Failed to load schemas"); return; }
            setSchemas(schemaNames);
            if (!schemaNames.includes("ScholarlySchema")) setSchemaName(schemaNames[0] ?? "");
        });
    }, []);
    ...
    const handleSubmit = async () => {
        ...
        if (error || !ws) {
            setSubmitError(
                (error as { detail?: string })?.detail ?? "Failed to create workspace",
            );
            setSubmitting(false);
            return;
        }
    ...
    const canSubmit = name.trim().length > 0 && schemas !== undefined && !submitting;

    return (
            <Root>
                <Panel className="w-160 flex flex-col gap-6">
                    ...
                    <div className="flex flex-col gap-3">
                        <h2 className="text-nord3 text-xs font-semibold uppercase tracking-widest">
                            Workspace Details
                        </h2>
                    ...
                    <InviteMembersSection invites={pendingInvites} onChange={setPendingInvites} currentUserEmail={currentUser?.email} />
                    {submitError && <p className="text-nord11 text-sm">{submitError}</p>}
